...
/* global UI DBWorker LibraryController BasicConnection BasicNode NotificationManager TranslateManager LibrariesEditor LogicController*/
...
function init() {
	...
	APP.htmlScript = document.getElementById("container");
	APP.htmlSVG = document.getElementById("containerSVG");
	
	APP.UI = new UI(true, true, true);
	APP.TM = new TranslateManager();
	APP.Logic = new Logic();
	
	APP.libraryEditor = new LibrariesEditor();
	APP.libraryController = new LibraryController();
	
	APP.logicController = new LogicController(APP.UI.htmlContainerLogics, APP.UI.htmlContainerStruct);
	
	APP.dbWorker = new DBWorker();
...
*/
	...
	removeObject(object) {
		...
		const logic = APP.logicController.currentLogic;
		if (!logic) {
			return;
		}
		
		
		if (logic.nodes[object.id]) {
			delete logic.nodes[object.id];
		} else if (logic.connections[object.id]) {
			delete logic.connections[object.id];
		}

		if (object.oid) {
			let find = false;
			// eslint-disable-next-line guard-for-in
			for (const nID in logic.nodes) {
				const n = logic.nodes[nID];
				if (!n) {
					continue;
				}
			...
			if (!find) {
				const index = logic.oids.indexOf(object.oid);
				if (index !== -1) {
					logic.oids.splice(index, 1);
				}
			}
	...
	saveScript() {
		...
		const object = APP.logicController.save();
		
		object.store = this.stores;
		// object.jsBlocksOIDs = this.listBlockOids;
		object.jsLibsOIDs = APP.libraryController.m_librariesOIDs;

		script = object;
		APP.saveScript = object;
		APP.script = APP.logicController.fillingScript(object);
	...
	dublicate(event, objects) {
		...
		for (const object of objects) {
			...
			const block = APP.logicController.currentLogic.createBlock(dublicateNode);
			if (block) {
				APP.UI.nodesScene.appendChild(block.htmlElement);
			}
	...
	copy(objects) {
		...
		for (const object of objects) {
			...
			this.copyX = Math.min(this.copyX, object.x);
			this.copyY = Math.min(this.copyY, object.y);
			this.bufferNodes.push(object);
		}
	}
	...
	addToCopied(node) {
		...
		this.bufferNodes.push(node);
	}

	paste(event) {
		
		if (!this.bufferNodes || !this.bufferNodes.length) {
			return;
		}
		
		const x = APP.UI.getSceneOffset(event, 0);
		const y = APP.UI.getSceneOffset(event, 1);
		
		const deltaX = this.copyX - x;
		const deltaY = this.copyY - y;
		
		const idsArr = [];
		const ids = {};
		const newBlocks = {};
		for (const block of this.bufferNodes) {
			...
			const copiedJson = this.generateNode(block);
			copiedJson.x = copiedJson.x - deltaX;
			copiedJson.y = copiedJson.y - deltaY;
			copiedJson.id = ++APP.index;
			const copiedBlock = APP.logicController.currentLogic.createBlock(copiedJson);
			if (copiedBlock) {
		...
		for (const block of this.bufferNodes) {
			...
			for (const connection of block.lines) {
				if (!connection) {
					continue;
				}
				...
				if (idsArr.indexOf(nId1) !== -1 && idsArr.indexOf(nId2) !== -1) {
					...
					const newIdConn = String(newNId1) + "_" + connection.index1 + "_" + newNId2 + "_" + connection.index2 + "_" + connection.type;
					if (idsConn.indexOf(newIdConn) === -1) {
						idsConn.push(newIdConn);
					} else {
						continue;
					}
	...
	saveFile() {
		...
		tt = tt.replace(":", ".");
		tt = tt.replace(":", ".");
		//const name = "save_" + dd + "_" + tt + ".js";
		const scriptName = "script.js";
		const data = "script = " + JSON.stringify(script);
		...
		const formName = "form.js";
		const formData = "form = " + JSON.stringify(APP.form);

		var element2 = document.createElement('a');
		element2.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(formData));
		element2.setAttribute('download', formName);
		...
		let request = '{"_id": {"$in": [';
		for (let i = 0; i <  script.jsBlocksOIDs.length; i++) {
			request += '{"$oid": "' + script.jsBlocksOIDs[i] + '"},';
		}
		...
		APP.dbWorker.responseDOLMongoRequest = this.loadBlocksByOids.bind(this);
		APP.dbWorker.sendBaseRCRequest("DOLMongoRequest","objects",request);
	}

	loadBlocksByOids(resultJSON){
		console.log("resultJSON",resultJSON);
		const libs = {};
		const data = resultJSON.cursor.firstBatch;
		for(let i = 0; i < data.length; i++){
			if(!libs.hasOwnProperty(data[i].object.lib["$oid"])){
				libs[data[i].object.lib["$oid"]] = [];
			}
			libs[data[i].object.lib["$oid"]].push(data[i].object.func);
		}
		console.log("libs",libs);
		
		const name = "library.js";

		var element = document.createElement('a');
		element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent("libsData = " + JSON.stringify(libs)));
	...
	fixNodeSemicolon()
	{
		...
		for(let i = 0; i < script.nodes.length; i++)
		{
			...
			{
				if(typeof script.nodes[i].Defaults.Inputs[j].value === 'string' && script.nodes[i].Defaults.Inputs[j].value.includes("/semicolon"))
				{
					let str = script.nodes[i].Defaults.Inputs[j].value.replaceAll("/semicolon",";");
	...
	replaceNodeSemicolon()
	{
		...
		for(let i = 0; i < script.nodes.length; i++)
		{
			...
			for(let j = 0; j < script.nodes[i].Defaults.Inputs.length; j++)
			{
				if(typeof script.nodes[i].Defaults.Inputs[j].value === 'string' && script.nodes[i].Defaults.Inputs[j].value.includes(";"))
				{
					let str = script.nodes[i].Defaults.Inputs[j].value.replaceAll(";","/semicolon");
					script.nodes[i].Defaults.Inputs[j].value = str;
				}
	...
	drop(event) {
		...
		APP.logicController.drop(event);
	...
	exportScript(jsonObject) {
		...
		for (const logic of logics) {
			...
			for (const node of logic.nodes) {
				...
				const nodeData = APP.libraryController.getNodeData(node.library, node.type);
				let nodeObject = {};
					...
					for (const input of node.inputs) {
						...
						if ("source_node" in input && "source_output" in input) {
							...
							for (const slot of srcNodeNew.slots) {
								if (slot.name === input.source_output &&
									slot.side === "OUT" &&
									slot.type === "data") {
									srcSlot = slot;
								} else if (srcSlot === null &&
									slot.side === "OUT" &&
									slot.type === "data") {
									srcSlot = slot;
								}
...
function decode_base64(s) {
	var b = l = 0, m = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
