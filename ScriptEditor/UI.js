...
/* global Slot APP sendPostRequest*/
"use strict";
...
class UI {
  constructor(left, top, right) {
    ...
    this.namespaceURL = decodeURIComponent(
      escape(window.atob("aHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmc="))
    );
    ...
    const _clipPathTemlate1 = document.createElementNS(
      this.namespaceURL,
      "clipPath"
    );
    ...
    _pathTemlate1.setAttribute(
      "d",
      `M 0 0 L 0 41 L 200 41 L 200 41 C 122 0 122 0 122 0 L 0 0 Z`
    );
    _pathTemlate1.setAttribute(
      "d",
      `M 0 0 L 0 20 L 200 20 L 200 20 C 122 0 122 0 122 0 L 0 0 Z`
    );
    _clipPathTemlate1.appendChild(_pathTemlate1);

    // BOTTOM
    let _clipPathTemlate2 = document.createElementNS(
      this.namespaceURL,
      "clipPath"
    );
    ...
    _pathTemlate2.setAttribute(
      "d",
      `M 0 0 L 200 0 L 200 20 L 78 19 C 78 19 78 19 0 0 L 0 0`
    );
  ...
  connectBrowserEvents() {
    ...
    const mousewheelEvnt = /Firefox/i.test(navigator.userAgent)
      ? "DOMMouseScroll"
      : "mousewheel";
    if (document.attachEvent) {
      ...
      // WC3 browsers
      document.addEventListener(mousewheelEvnt, this.mouseWheel.bind(this));
    }
  }
  ...
  mouseDown(event) {
    ...
    // CHECK HOVER OBJECT
    if (event.target.offsetParent !== APP.UI.rightPanel) {
      this.hovered = this.checkHover(event);
      if (!this.hovered) {
        APP.UI.rightPanel.remove();
        APP.UI.createRight();
      } else if (this.focused[0]) {
        if (this.focused[0] !== this.hovered) {
          APP.UI.rightPanel.remove();
          APP.UI.createRight();
        }
      }
    ...
    if (
      ...
      (targetClass === "container" ||
        targetClass === "ViewView" ||
        targetClass === "ViewArea" ||
  ...
  mouseUp(event) {
    ...
    if (this.selecedArea) {
      ...
      const { currentLogic } = APP.logicController;
      if (currentLogic) {
        for (const nId in currentLogic.nodes) {
          const n = currentLogic.nodes[nId];
          if (!n) continue;
  ...
  checkScenePosition(dx, dy) {
    ...
    // GET UP LEFT POINT
    // const px = APP.Logic.logicBlocksStructs[0].html.offsetLeft;
    // const py = APP.Logic.logicBlocksStructs[0].html.offsetTop;

    const px = -400;

    const vec = this.translatePosFrom(px, 0);
    const controlX = vec[0];
    // const controlY = vec[1];
  ...
  getSceneOffsetXY(x, y) {
    const w = window.innerWidth / 2;
    const resW = (x - w) / this.camZ + w + this.camX;
    const h = window.innerHeight / 2;
    const resH = (y - h) / this.camZ + h + this.camY;
    return [resW, resH];
  }

  updateScene() {
    this.nodesScene.style.transform = `scale(${this.camZ}) translate(${-this
      .camX}px, ${-this.camY}px)`;
  }
  ...
  checkHover(event) {
    ...
    for (const n of path) {
      ...
      if (n.id.indexOf("N_") !== -1) {
        ...
        return currentLogic.connections[id];
      }
    }
  ...
  createFormEditorButton() {
    ...
    this.b6 = this.createBtn(
      ...
      function f3() {
        ...
        let formEditorContainer = document.getElementById(
          "formEditorContainer"
        );
  ...
  createExpandDefaultsButton() {
    ...
    this.b10 = this.createBtn(
      this.topBar,
      "ExpandDefsBtn",
      "EXPAND DEFAULTS",
      function f3() {
        if (that.b10.textContent === "EXPAND DEFAULTS") {
          for (let defaultBlock of document.getElementsByClassName(
            "StoreBlock"
          ))
  ...
  createTop() {
    ...
    this.createBtn(
      ...
      "TopRunBtn",
      "RUN",
      this.restartApp.bind(this)
    );
    this.createBtn(
      this.topBar,
      "TopLibsBtn",
      "LIBS",
      this.libEditor.bind(this)
    );
    ...
    this.topBar.appendChild(this.divObjectName);
  }
  ...
  restartApp() {
    APP.Logic.saveScript();
    if (window.thManager !== null) {
      window.thManager.closeWebSocket();
      window.thManager = null;
    }
  ...
  createTopRightBar() {
    ...
    if (APP.auth) {
      ...
      this.btnReg.addEventListener(
        "click",
        this.openRegistrationPanel.bind(this)
      );
    }
    ...
    console.log('APP', APP);
    langBox.addEventListener(
      "change",
      APP.TM.changeLanguageEvent.bind(APP.TM)
    );
    rightTopDiv.appendChild(langBox);

    for (const lang of APP.TM.langs) {
  ...
  getStoreList(slot) {
    ...
    if (this.nodeSlot) {
      ...
      if (this.nodeSlot.defaults && !this.nodeSlot.store) {
  ...
  acceptStoreDefaults(event) {
    if (this.nodeSlot && this.nodeSlot instanceof Slot) {
      if (this.storeSelected) {
        ...
        if (currentLogic) {
          const def =
            this.storeContainer.children[0].children[1].children[2].value;
          currentLogic.nodes[this.nodeSlot.nodeID].appendDefaults(
            this.nodeSlot,
            {
  ...
  search(e) {
    ...
    if (!isNaN(value)) {
      // search by ID
      node = currentLogic.nodes[parseInt(value)];
    } else {
      // search by str
      // eslint-disable-next-line guard-for-in
      for (let nodeID in currentLogic.nodes) {
        let n = currentLogic.nodes[nodeID];
        if (!n) {
          continue;
        }
        if (
          n.type.toLowerCase().indexOf(value.toLowerCase()) !== -1 ||
  ...
  searchBlock(block) {
    ...
    this.camZ = 2;
    const nx = block.htmlElement.offsetLeft;
    const ny = block.htmlElement.offsetTop;
    const nw = block.htmlElement.offsetWidth / 2;
    const nh = block.htmlElement.offsetHeight / 2;

    const vec = this.translatePosToCenter(nx + nw, ny + nh);
    this.camX = vec[0];
    this.camY = vec[1];
  ...
  openLoginPanel() {
    ...
    loginTitleText.textContent = "Login Account";
  ...
  sendLoginClick() {
    ...
    const data =
      "username=" +
      encodeURIComponent(login.value) +
      "&password=" +
  ...
  callbackLoginRequest(event) {
    ...
    if (successStatus === event.target.responseText) {
      ...
      sendPostRequest(
        "/user/status",
        "",
        APP.dbWorker.onCheckUserStatus.bind(APP.dbWorker)
      );
  ...
  openRegistrationPanel() {
    ...
    passText2.textContent = "Confirm password";
  ...
  openScripsList() {
    ...
    closeListScripts.addEventListener(
      "click",
      function fff() {
  ...
  saveDialog() {
    ...
    saveDialogTitle.textContent = "Save Application Object";
    ...
    btnSave.addEventListener(
      ...
      function fSave(
        ...
        if (wiki.length === 0) {
          valid = false;
          wikiText.textContent = "Wiki Ref: {field is empty}";
  ...
  sendSaveRequest() {
    ...
    try {
      ...
      APP.log("warn", "APP.Logic.replaceNodeSemicolon(); -<== ERROR");
    }

    const saveObjectData = JSON.stringify({
      script_data: APP.saveScript,
      form_data: APP.form,
    });

    const b64SaveData = b64EncodeUnicode(saveObjectData);

    // CHECK VALID SAVE
    try {
      JSON.parse(b64DecodeUnicode(b64SaveData));
    } catch (e) {
      APP.log("error", "Bad save data!");
      if (confirm("Script saved BAD DATA! Anyway SAVE?")) {
        APP.log("warn", "Continuation save!");
      } else {
        return;
      }
    }
  ...
  editValue(name, slot) {
    ...
    if (name) {
      // store
      baseValue = APP.Logic.stores[name];
    } else if (slot.defaults) {
      // defaults
      baseValue = slot.defaults.value;
    }
    ...
    if (storeType === "boolean") {
      ...
      inpO.click();
      document.getElementById("ValueEditInput").value =
  ...
  createLeft() {
    ...
    this.m_btnControlHtml.addEventListener(
      "click",
      this.controlLeftPanel.bind(this)
    );
    ...
    this.htmlBtnObjects = document.createElement("div");
    this.htmlBtnObjects.textContent = "Objects";
    this.htmlBtnObjects.addEventListener("click", this.controlTab1.bind(this));
    htmlTab1BtnsContainer.appendChild(this.htmlBtnObjects);

    this.htmlBtnWidgets = document.createElement("div");
    this.htmlBtnWidgets.textContent = "Vidgets";
    this.htmlBtnWidgets.addEventListener("click", this.controlTab1.bind(this));
    htmlTab1BtnsContainer.appendChild(this.htmlBtnWidgets);

    this.htmlBtnLibraries = document.createElement("div");
    this.htmlBtnLibraries.textContent = "Libs";
    this.htmlBtnLibraries.addEventListener(
      "click",
      this.controlTab1.bind(this)
    );
    htmlTab1BtnsContainer.appendChild(this.htmlBtnLibraries);
    ...
    this.htmlBtnLogics = document.createElement("div");
    this.htmlBtnLogics.textContent = "Logic";
    this.htmlBtnLogics.addEventListener("click", this.controlTab2.bind(this));
    htmlTab2BtnsContainer.appendChild(this.htmlBtnLogics);

    this.htmlBtnStruct = document.createElement("div");
    this.htmlBtnStruct.textContent = "Structure";
    this.htmlBtnStruct.addEventListener("click", this.controlTab2.bind(this));
    htmlTab2BtnsContainer.appendChild(this.htmlBtnStruct);
    ...
    this.htmlContainerStruct.style.display = "none";
  ...
  controlLeftPanel() {
    if (this.leftPanel.className === "LeftPanelView") {
      ...
      this.m_btnControlHtml.textContent = "Show";
    } else {
      this.leftPanel.className = "LeftPanelView";
      this.m_btnControlHtml.textContent = "Hide";
    }
  ...
  controlTab1(event) {
    if (event.target === this.htmlBtnObjects) {
      this.htmlBtnObjects.classList.add("selected");
      this.htmlBtnWidgets.classList.remove("selected");
      this.htmlBtnLibraries.classList.remove("selected");
      this.htmlContainerObjects.style.display = "";
      this.htmlContainerWidgets.style.display = "none";
      this.htmlContainerLibraries.style.display = "none";
    } else if (event.target === this.htmlBtnWidgets) {
      this.htmlBtnObjects.classList.remove("selected");
      this.htmlBtnWidgets.classList.add("selected");
      this.htmlBtnLibraries.classList.remove("selected");
      this.htmlContainerObjects.style.display = "none";
      this.htmlContainerWidgets.style.display = "";
      this.htmlContainerLibraries.style.display = "none";
    } else if (event.target === this.htmlBtnLibraries) {
      this.htmlBtnObjects.classList.remove("selected");
      this.htmlBtnWidgets.classList.remove("selected");
      this.htmlBtnLibraries.classList.add("selected");
      this.htmlContainerObjects.style.display = "none";
      this.htmlContainerWidgets.style.display = "none";
      this.htmlContainerLibraries.style.display = "";
    }
  }

  controlTab2(event) {
    if (event.target === this.htmlBtnLogics) {
      this.htmlBtnLogics.classList.add("selected");
      this.htmlBtnStruct.classList.remove("selected");
      this.htmlContainerLogics.style.display = "";
      this.htmlContainerStruct.style.display = "none";
    } else {
      this.htmlBtnLogics.classList.remove("selected");
      this.htmlBtnStruct.classList.add("selected");
      this.htmlContainerLogics.style.display = "none";
      this.htmlContainerStruct.style.display = "";
    }
  }
  ...
  createRight() {
    ...
    this.rightPanel.className = "RightPanelView";
    this.rightPanel.style.display = "none";
    document.body.appendChild(this.rightPanel);

    // HIDE / SHOW
    this.m_btnsRightContainerHtml = document.createElement("div");
    this.m_btnsRightContainerHtml.id = "RightPanelControlBtn";
    this.m_btnsRightContainerHtml.textContent = "HIDE";
    this.m_btnsRightContainerHtml.addEventListener(
      "click",
      this.controlRightPanel.bind(this)
    );
    this.rightPanel.appendChild(this.m_btnsRightContainerHtml);

    this.m_rightPanelContainerHtml = document.createElement("div");
    this.m_rightPanelContainerHtml.className = "RVMainContainer";
    this.rightPanel.appendChild(this.m_rightPanelContainerHtml);
  }

  // eslint-disable-next-line no-dupe-class-members
  controlRightPanel() {
    if (this.rightPanel.className === "RightPanelView") {
      this.rightPanel.className = "RightPanelHide";
      this.m_btnsRightContainerHtml.textContent = "HIDE";
    } else {
      this.rightPanel.className = "RightPanelView";
      this.m_btnsRightContainerHtml.textContent = "SHOW";
    }
  }
}
