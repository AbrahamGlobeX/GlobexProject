...
class DBWorker {
	...
	closeSocketConnection(event) {
		...
		APP.log("error", "Соединение прервано");
		this.m_socket = null;
		this.connectToWebSocketServer();
	}
	...
	sendInsertRCRequest(id, objectData, collection = "objects") {
	...
	sendUpdateRCRequest(id, oid, objectSetsData, collection = "objects") {
	...
	responseGetApplicationObject(resultJson) {
		...
		if ("cursor" in resultJson) {
			...
			try {
				...
				APP.log("warn", "APP.Logic.fixNodeSemicolon(); -<== ERROR");
			}
			...
			APP.script = APP.logicController.fillingScript(script);
...
function sendPostRequestMultiPart(api,data,callbackFunc){
	const request = new XMLHttpRequest;
	request.open("POST","https://guni.0070.ru" + api, true);
	request.withCredentials = true;
