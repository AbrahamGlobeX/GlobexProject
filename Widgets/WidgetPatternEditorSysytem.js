class PatternEditorSystem extends BaseObjectEditor {
    ...
    openPrototypeProperties(pattern, mode = "edit", props = undefined) {
        try {
                ...
                () => {
                    if (mode === "view") {
                        ...
                        for (let i = 0; i < propsArray.length; i++) {
                            pattern.schema.properties[propsArray[i]._name] = {}
                            pattern.schema.properties[propsArray[i]._name].average_default = 0;
                            pattern.schema.properties[propsArray[i]._name].count_uses = 0;
                            pattern.schema.properties[propsArray[i]._name].category = "Characteristics"
                            if (propsArray[i]._unit) pattern.schema.properties[propsArray[i]._name].current_unit = propsArray[i]._unit;
                            pattern.schema.properties[propsArray[i]._name].current_system = "SI"
                            if (propsArray[i]._desc) pattern.schema.properties[propsArray[i]._name].description = propsArray[i]._desc;
                            if (propsArray[i]._lang) pattern.schema.properties[propsArray[i]._name].lang = propsArray[i]._lang;
                            pattern.schema.properties[propsArray[i]._name].prop_ref = {}
                            if (propsArray[i]._propRef) if (propsArray[i]._propRef["$oid"]) pattern.schema.properties[propsArray[i]._name].prop_ref["$oid"] = propsArray[i]._propRef["$oid"];
                            pattern.schema.properties[propsArray[i]._name].themes = ["Basic"];
