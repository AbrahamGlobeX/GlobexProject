class PatternEditorSystem extends BaseObjectEditor {
    ...
    drawCategoryProperties(parentLayout, categoryName, isEmpty) {
        const categoryLayout = this.drawLayout(parentLayout, "layoutHorizontal", { "width": "100%" });
        const name = this.drawLabel(categoryLayout, categoryName);

        ReactComponent[name].controlHorizontalTextAlign(1);
        if (!isEmpty) {
            const descLayout = this.drawLayout(parentLayout, "layoutHorizontal", { "width": "90%", "marginLeft": "10%" });
    ...
    drawButtonAddProperty(parentLayout, callback) {
        return this.drawButton(
            this.drawLayout(
                parentLayout,
                "layoutHorizontal",
                { "width": "5%", "marginLeft": "95%", "minHeight": "50px", "maxHeight": "50px", "marginTop": "3%", "background": "#e8e8e8" }
            ),
            "+",
            { "color": "#123456" },
            callback);
    ...
    createCategories(pattern) {
        ...
        this._categoriesWithList[pattern]["Characteristics"] = CategoryWithProperties.create({ "en": "Characteristics", "ru": "Характеристики" });
        this._categoriesWithList[pattern]["Location"] = CategoryWithProperties.create({ "en": "Location", "ru": "Местоположение" });
    }
    openPatternCreateForm(name,category,callback){
        ...
        const titleLayout = this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%" });
        this.drawLabel(titleLayout, "Создание прототипа ");

        const categoriesWithPropertiesLayout = {};

        this.drawButton(this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" }),
            "Добавить свойство",
            { "color": "#123456" },
            () => {
                const editor = new Property_Editor();
                editor.addNewProperty = (...args) => {
                    this.addNewProperty.bind(this, args, categoriesWithPropertiesLayout[args[4]])();
                }
            }
        );
        for (let category of Object.keys(this._categoriesWithList[patternName])) {
            const properties = this._categoriesWithList[patternName][category].properties;
            categoriesWithPropertiesLayout[category] = this.drawCategoryProperties(mainLayout, this._categoriesWithList[patternName][category].getName("ru"), !(properties.length > 0));
            for (let property of properties) {
                this.drawPropertyItem(categoriesWithPropertiesLayout[category], property, "edit");
            }
            this.drawButtonAddProperty(categoriesWithPropertiesLayout[category], () => {
                const editor = new Property_Editor();
                editor.addNewProperty = (...args) => {
                    this.addNewProperty.bind(this, args, categoriesWithPropertiesLayout[category])();
                }
            })
        }
    ...
    openObjectFormEdit(object, props, pattern, callback) {
        ...
        this.createCategories(patternName);
        const seperated = CategoryWithProperties.seperatedPropertiesByGroup(properties);

        for (let category of Object.keys(seperated)) {
            this._categoriesWithList[patternName][category].addProperties(seperated[category], true);
        }
        ...
        const titleLayout = this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%" });
        this.drawLabel(titleLayout, "Редактирование объекта " + object["meta"]["name"]);
        ...
        () => {
            ...
            const newMainLayout = this.drawLayout(newDialog,"layoutVertical", {"minWidth" : "90vw"});
            ...
            const contentLayout = this.drawLayout(newMainLayout,"layoutHorizontal",{"width" : "100%"});
            ReactComponent[contentLayout].includeWidget(this.smartWidget);
            const btnLayout = this.drawLayout(newMainLayout,"layoutHorizontal",{"width" : "100%"});
    ...
    async createPrototype(callback){
        const categoriesWithProperties = this._categoriesWithList[Object.keys(this._categoriesWithList)[0]];
        const properties = {};
        const newProperties = {};
        ...
        for (let category of Object.keys(categoriesWithProperties)) {
            ...
            newProperties[category] = properties[category].filter((item) => !item.propRef);
        }
        ...
        for (let category of Object.keys(properties)) {
            ...
            for (let property of properties[category]) {
                if (!this._selectedPattern["schema"]["properties"].hasOwnProperty(property.name) &&
                    !this._selectedPattern["schema"]["unverified"].hasOwnProperty(property.name)) {
        ...
        await this.addPropsToOwnPattern(this._selectedPattern["_id"]["$oid"],addPropsToPattern);
    ...
    async saveObject(object, callback) {
        ...
        await this.addPropsToPattern(this._selectedPattern["_id"]["$oid"], addPropsToPattern);
        ...
        for (let category of Object.keys(properties)) {
            ...
            if(propertyNames.length != 0){
                const delProp = Object.keys(object["object"]).filter((item) => propertyNames.indexOf(item) == -1);
    ...
    openPrototypeProperties(pattern, mode = "view", props = undefined) {
        try {
            ...
            const l1 = CategoryWithProperties.seperatedPropertiesByGroup(pattern["schema"]["properties"]);

            for (let category of Object.keys(l1)) {
                this._categoriesWithList[patternName][category].addProperties(l1[category]);
            }


            const categoriesWithPropertiesLayout = {};
            const mainLayout = this.drawLayout(this._viewPatternPropertiesDialog, "layoutVertical", { "minWidth": "50vw", "minHeight": "75vh", "maxHeight": "75vh" });

            this.drawLabel(this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" }), "Просмотр свойств прототипа");
            this.drawLabel(this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" }), pattern["meta"]["name"]);
            ...
            for (let category of Object.keys(this._categoriesWithList[patternName])) {
                ...
                categoriesWithPropertiesLayout[category] = this.drawCategoryProperties(mainLayout, this._categoriesWithList[patternName][category].getName("ru"), !(properties.length > 0));
                for (let property of properties) {
                    this.drawPropertyItem(categoriesWithPropertiesLayout[category], property, mode);
                }
            ...
            const btnLayout = this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%" });
            this.drawButton(btnLayout, mode === "view" ? "Создать объект" : "Сохранить", { "color": "#123456" }, () => {
                if (mode === "view") {
            ...
            this.drawButton(btnLayout, mode === "view" ? "Закрыть" : "Отмена", { "color": "#123456" }, () => {
                if (mode === "view") {
                    ReactComponent[this._viewPatternPropertiesDialog].destroyWidget();
                    this._viewPatternPropertiesDialog = undefined;
                    this._selectedPropertiesNewVersion = {};
    ...
    addNewProperty(arr, layout) {
        const [lang, name, desc, wiki, category, valueType, value, unitType, unit, propRef, parentPattern] = arr;
        const property = this._categoriesWithList[this._selectedPattern["meta"]["name"]][category].addProperty(propRef, name, lang, desc, wiki, valueType, value, true, unitType, unit);
        this.drawPropertyItem(layout, property, "edit");
        ReactComponent[layout].childGoToLast(ReactComponent[layout].children.length - 2);
    }
    openFormSetObjectName(callback) {
        const dialog = this.drawDialog(-1);
        const mainLayout = this.drawLayout(dialog, "layoutVertical", { "minWidth": "500px" });
        this.drawLabel(this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" }), "Создание объекта");

        const nameLayout = this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" });
        this.drawLabel(nameLayout, "Имя объекта");
        let nameInput = "";
        ReactComponent[this.drawInput(nameLayout, "")].htmlElement.oninput = (e) => {
            nameInput = e.target.value;
        };

        const wikiLayout = this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" });
        this.drawLabel(wikiLayout, "Википедия");
        let wikiInput = "";

        ReactComponent[this.drawInput(wikiLayout, "")].htmlElement.oninput = (e) => {
            wikiInput = e.target.value;
        };

        const btnLayout = this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" });
    ...
    async createObjectVersion2(name, wiki, callback) {
        ...
        await this.addPropsToPattern(this._selectedPattern["_id"]["$oid"], addPropsToPattern);
        await this.incPropsCount();
        const object = await this.createNewObject(this._selectedPattern, name, wiki, properties);
    ...
    async createNewProp(property) {
        return new Promise((res) => {
            ...
            const created = function (resultJSON) {
                ...
                APP.dbWorker.sendUpdateRCRequest("DOLMongoRequest", resultJSON.inserted_id["$oid"], JSON.stringify(sets), "props");
            }
            const object = {
                "average_default": 0,
                "category": property.category,
            ...
            APP.dbWorker.sendInsertRCRequest("DOLMongoRequest", JSON.stringify(object), "props");
        })
    }
    ...
    async createPattern(pattern){
        return new Promise((res) => {
            ...
            APP.dbWorker.responseDOLMongoRequest = created.bind(this);
            APP.dbWorker.sendInsertRCRequest("DOLMongoRequest",JSON.stringify(pattern),"patterns");
        })
    }
    async addPropsToOwnPattern(patternID, props) {
        return new Promise((res) => {
            ...
            APP.dbWorker.sendUpdateRCRequest("DOLMongoRequest", patternID, JSON.stringify({
                "$set": sets
            }), "patterns");
        });
    }
        ...
        return new Promise((res) => {
            ...
            for (let category of Object.keys(props)) {
                for (let property of props[category]) {
                    ...
                    if(property.propRef){
                        sets["schema.unverified." + property.name]["prop_ref"] = property.propRef;
                    }
                }
            }
            APP.dbWorker.responseDOLMongoRequest = inserted.bind(this);
    ...
    async createNewObject(pattern, name, wiki, properties) {
        return new Promise((res) => {
            ...
            APP.dbWorker.responseDOLMongoRequest = inserted.bind(this);
            APP.dbWorker.sendInsertRCRequest("DOLMongoRequest", JSON.stringify(object));
        })
    }
    ...
    switchSearchMode() {
        ...
        const otherTree = this._searchMode == "object" ? MainClassification.getAllClassificationTree() : MainClassificator.getAllClassificatorTree();

        for (let id of Object.keys(otherTree)) {
            const layout2 = this.drawLayout(this._searchTreeLayout, "layoutHorizontal", { "width": "100%" });
            ReactComponent[layout2].includeWidget(otherTree[id]);
        }


        if (this._searchMode == "pattern") {
            this.drawButton(this._searchBtnLayout, "Открыть классификатор", { "color": "#123456" }, () => {
                MainClassificator.loadMainClassificator();
            });
            this.drawButton(this._searchBtnLayout, "Новый", { "color": "#123456" }, () => {

            });
        }

        this.drawButton(this._searchBtnLayout, "Закрыть", { "color": "#123456" }, () => {
    ...
    drawFormObjectsWithPatterns() {
        ...
        const mainLayout = this.drawLayout(this.dialogFormObjectsWithPatterns, "layoutVertical", { "minWidth": "35vw", "minHeight": "65vh" });

        this.drawLabel(this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" }), "Создайте по прототипу или выберете необходим Вам объект");

        const searchLayout = this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" });
        this.drawLabel(searchLayout, "Поиск");
        ReactComponent[this.drawInput(searchLayout, "")].htmlElement.oninput = this.controlFindInput.bind(this);
        const searchOptionLayout = this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" });
        const objectBTNOption = this.drawButton(searchOptionLayout, "Объекты", { "color": "#123456", "background": "grey" }, () => {
            if (currentSearchOptionButton) {
                this.widgetSetStyle(currentSearchOptionButton, { "background": "none" });
            }
        ...
        currentSearchOptionButton = objectBTNOption;
        const patternBTNOption = this.drawButton(searchOptionLayout, "Прототип", { "color": "#123456" }, () => {
            if (currentSearchOptionButton) {
                this.widgetSetStyle(currentSearchOptionButton, { "background": "none" });
            }
        ...
        this._searchLayout = this.drawLayout(this.drawLayout(mainLayout, "layoutHorizontal", { "width": "100%" }), "layoutVertical", { "height": "100%" });
        this._searchTreeLayout = this.drawLayout(this.drawLayout(this._searchLayout, "layoutHorizontal", { "width": "100%" }), "layoutVertical", { "height": "400px" });
        this._searchBtnLayout = this.drawLayout(this._searchLayout, "layoutHorizontal", { "width": "100%", "minHeight": "50px", "maxHeight": "50px" });
        this._searchMode = "object";
        this.switchSearchMode();
    ...
    controlFindInput(e) {
        ...
        if (this._searchMode == "object") {
            ...
            const tree = MainClassification.drawTreeByClassificationsWithObjects(data);

            for (let id of Object.keys(tree)) {
                const layout = this.drawLayout(this._searchTreeLayout, "layoutHorizontal", { "width": "100%" });
