...
class WidgetSmartImageView extends BaseWidget {
	...
	onCreate() {
			...
			"Name0": {
			  ...
			  "buttons": [
				...
				},
				{
				  ...
				  "obj": {
					"_id": {
					  "$oid": "60cad8681d8a270381a406e8"
					},
					"meta": {
					  "name": "TestObject.2",
					  "description": "SmartImage test object",
					  "pattern": {
						"$oid": "5ce777ef9ddcac72773c1a47"
					  },
					  "owner": {
						"$oid": "5fe0241d730fc50009605a1a"
					  }
					},
					"additional": {
					  "wiki_ref": {
						"en": "https://ru.wikipedia.org/wiki/Test"
					  },
					  "category": [
		...
		this.htmlElement.appendChild(this.mainMainLay.htmlElement);
		...
		window.addEventListener('resize', event => {
			if (ReactComponent["w_1"].htmlElement.parentNode.parentNode.style.display != "none") {this.oldX = this.htmlImage.style.width.substring(0, this.htmlImage.style.width.length - 2);
			this.oldY = this.htmlImage.style.height.substring(0, this.htmlImage.style.height.length - 2);
			this.htmlImage.style.height = this.svg.clientHeight + 'px';
			this.htmlImage.style.width = this.svg.clientHeight*this.c + 'px';
	...
	loadBase() {
		...
		Object.keys(obj).forEach(key => {
			if (obj[key].btn) {
				let index = this.saveData[this.name].buttons.map(function(e) { return e.id; }).indexOf(obj[key].btn);
				this.currentLayer = this.saveData[this.name].buttons[index].treeItem.id;
			}
			this.saveData[key] = {};
			this.loadWorkFromDatabase(key);
			
			if (key != "Name0") {
				this.target = this.saveData["Name0"].buttons[0];
				this.createBuild(key);
			}
			this.createNewLayer();
		});
	...
	loadWorkFromDatabase(key) { 


		let obj = this.loading;
		for (let i = 0; i < obj[key].buttons.length; i++) {
			let btn = obj[key].buttons[i];
			this.createBtn();
			this.setNumPointers(this.btnData.btnArray[i], obj[key].buttons[i].tempBtn.length - 1);
			this.btnData.btnArray[i].setAttribute("id", btn.id);
			this.btnData.btnArray[i].setAttribute("x", btn.pos.x);
			this.btnData.btnArray[i].textElement.setAttribute("x", Number(btn.pos.x) + Number(this.btnData.btnArray[i].clientWidth/2 ));
			this.btnData.btnArray[i].textElement.setAttribute("y", Number(btn.pos.y) + Number(this.btnData.btnArray[i].clientHeight/2));
			this.btnData.btnArray[i].setAttribute("y", btn.pos.y);
			this.btnData.btnArray[i].setAttribute("rx", btn.pos.rx);
			this.btnData.btnArray[i].setAttribute("ry", btn.pos.ry);
			if (btn.obj) this.btnData.btnArray[i].obj = btn.obj;
			for (let j = 0; j < obj[key].buttons[i].tempBtn.length; j++) { 
				this.btnData.btnArray[i].tempBtn[j].setAttribute('cx', btn.tempBtn[j].pos.x);
				this.btnData.btnArray[i].tempBtn[j].setAttribute('cy', btn.tempBtn[j].pos.y);
				this.btnData.btnArray[i].tempBtn[j].currentLine.setAttribute('x1', Number(btn.pos.x) + Number(this.btnData.btnArray[i].clientWidth/2 ))
				this.btnData.btnArray[i].tempBtn[j].currentLine.setAttribute('y1', Number(btn.pos.y) + Number(this.btnData.btnArray[i].clientHeight/2))
				this.btnData.btnArray[i].tempBtn[j].currentLine.setAttribute('x2', this.btnData.btnArray[i].tempBtn[j].attributes.cx.value);
				this.btnData.btnArray[i].tempBtn[j].currentLine.setAttribute('y2', this.btnData.btnArray[i].tempBtn[j].attributes.cy.value);
			}
			for (let j = 0; j < obj[key].buttons[i].duplicates.length; j++) {
				this.duplicateButton(this.btnData.btnArray[i]);
				this.setNumPointers(this.btnData.btnArray[i].duplicates[j], obj[key].buttons[i].duplicates[j].tempBtn.length);
				this.btnData.btnArray[i].duplicates[j].setAttribute("id", btn.duplicates[j].id);
				this.btnData.btnArray[i].duplicates[j].setAttribute("x", btn.duplicates[j].pos.x);
				this.btnData.btnArray[i].duplicates[j].textElement.setAttribute("x", Number(btn.duplicates[j].pos.x) + Number(this.btnData.btnArray[i].duplicates[j].clientWidth/2 ));
				this.btnData.btnArray[i].duplicates[j].textElement.setAttribute("y", Number(btn.duplicates[j].pos.y) + Number(this.btnData.btnArray[i].duplicates[j].clientHeight/2));
				this.btnData.btnArray[i].duplicates[j].setAttribute("y", btn.duplicates[j].pos.y);
				this.btnData.btnArray[i].duplicates[j].setAttribute("rx", btn.duplicates[j].pos.rx);
				this.btnData.btnArray[i].duplicates[j].setAttribute("ry", btn.duplicates[j].pos.ry);
				for (let k = 0; k < obj[key].buttons[i].duplicates[j].tempBtn.length; k++) { 
					this.btnData.btnArray[i].duplicates[j].tempBtn[k].setAttribute('cx', btn.duplicates[j].tempBtn[k].pos.x);
					this.btnData.btnArray[i].duplicates[j].tempBtn[k].setAttribute('cy', btn.duplicates[j].tempBtn[k].pos.y);
					this.btnData.btnArray[i].duplicates[j].tempBtn[j].currentLine.setAttribute('x1', Number(btn.duplicates[j].pos.x) + Number(this.btnData.btnArray[i].clientWidth/2 ))
					this.btnData.btnArray[i].duplicates[j].tempBtn[j].currentLine.setAttribute('y1', Number(btn.duplicates[j].pos.y) + Number(this.btnData.btnArray[i].clientHeight/2))
					this.btnData.btnArray[i].duplicates[j].tempBtn[j].currentLine.setAttribute('x2', this.btnData.btnArray[i].duplicates[j].tempBtn[j].attributes.cx.value);
					this.btnData.btnArray[i].duplicates[j].tempBtn[j].currentLine.setAttribute('y2', this.btnData.btnArray[i].duplicates[j].tempBtn[j].attributes.cy.value);
				}
			}
		...
		this.saveData[key].buttons = this.btnData.btnArray;
	}
	...
	refreshList(no = false) {
		...
		if (!no) {
			...
			laylay.htmlElement.style.height = "13%";
	...
	setNumPointers(btn, num) {
		for (let i = 0; i < num; i++) {
			...
			line.setAttribute("x1", btn.x.animVal.value + btn.clientWidth/2  + '');
		...
		if (btnNew.styles.r == 0) btnNew.style.width = btnNew.styles.height.substring(0, btnNew.style.height.length - 2)*2 + 'px';
	...
	linkObject(value) { //=-= заргузка объекта
		const dialog = new widgetsComponentsTypes["dialog"];
		dialog.dialogContent.style.width = "700px";
		dialog.dialogContent.style.height = "300px";
		const input = new widgetsComponentsTypes["input"];
	...
	buttonChooseEdit(value) {//=-= отладка
		...
		btn2.addEventListener("mousedown", e => {
			...
			if (withNumber.checked) example[0].innerText = example[0].num + '. ' + example[0].value;
		...
		if (this.target == "") {dialog.addDialogButton("Сохранить", () => {
	...
	createBuild(e = null) {
		this.name = e;
		if (!e) this.target.layerName = "Name" + this.layers.length;
		else this.target.layerName = e;
...
openLayer() {
	...
	if (this.target.obj) this.showProps(this.target.obj);
}
	...
	onMouseMove(e) {
		...
		if (this.editMode) for (let i = 0; i < this.btnData.btnArray.length; i++) {
			...
			if (btn.mouse) if (btn.mouse.move && (btn.x.animVal.value + e.clientX - btn.mouse.x > 1) && (btn.y.animVal.value + e.clientY - btn.mouse.y > 1)) {
				...
				btn.textElement.setAttribute("x", btn.x.animVal.value + btn.clientWidth/2 + e.clientX - btn.mouse.x  - 8);
				btn.textElement.setAttribute("y", btn.y.animVal.value + btn.clientHeight/2 + e.clientY - btn.mouse.y + 6);
	...
	set imageData(value) {
		...
		this.htmlImage.style.backgroundImage = `url(${value})`;
	}
