class WidgetLayoutHorizontal extends WidgetLayout {
  ...
  set spacing(value) {
    ...
    if (this.htmlElement.children.length > 1) {
      let lastChild =
        this.htmlElement.children[this.htmlElement.children.length - 1];
      if (lastChild != null) lastChild.style.marginRight = null;
    }
  }
  ...
  controlChildrenHorizontalAlignType(alignmentType) {
    ...
    if (alignmentType === 0) this.htmlElement.style.justifyContent = "";
  ...
  controlChildrenVerticalAlignType(alignmentType) {
    ...
    if (alignmentType === 1) this.htmlElement.style.alignItems = "flex-start";
    else if (alignmentType === 2) this.htmlElement.style.alignItems = "center";
    else if (alignmentType === 3)
  ...
  hasScroll() {
    return (
      Math.abs(this.htmlElement.scrollWidth - this.htmlElement.clientWidth) !==
      0
    );
  }
}
