...
class JSPart {
    ...
    createWidget(widgets, type) {
        ...
        let width = "500px";
        let height = "500px";
        if (type != "window" && type != "layout" && type != "layoutGrid" && type != "dialog") {
            width = "150px";
            height = "75px";
        }
        if (type == "tree") {
    ...
    createDialog(type, content)
    {
        ...
        switch(type)
        {
            case "CreateProject":
            {
                ...
                ReactComponent[mainlayout.id].includeWidget(ReactComponent[titlelayout.id]);
                returned["title"] = {
                ...
                ReactComponent[namelayout.id].includeWidget(ReactComponent[namelabel.id]);
                ReactComponent[namelayout.id].includeWidget(ReactComponent[nameinput.id]);
                ReactComponent[maincontentlayout.id].includeWidget(ReactComponent[namelayout.id]);
                ...
                ReactComponent[desclayout.id].includeWidget(ReactComponent[desclabel.id]);
                ReactComponent[desclayout.id].includeWidget(ReactComponent[descinput.id]);
                ReactComponent[maincontentlayout.id].includeWidget(ReactComponent[desclayout.id]);
                ...
                ReactComponent[wikilayout.id].includeWidget(ReactComponent[wikilabel.id]);
                ReactComponent[wikilayout.id].includeWidget(ReactComponent[wikiinput.id]);
                ReactComponent[maincontentlayout.id].includeWidget(ReactComponent[wikilayout.id]);

                let classificationsLayout = new widgetsComponentsTypes["layoutVertical"];
                for(let i = 0; i < content.length; i++)
                {
                    ...
                    ReactComponent[classnamelayot.id].includeWidget(ReactComponent[classlabel.id]);
                } 

                

                ReactComponent[mainlayout.id].includeWidget(ReactComponent[maincontentlayout.id]);
                ReactComponent[mainlayout.id].includeWidget(ReactComponent[classificationsLayout.id]);
                ...
                ReactComponent[buttonlayout.id].includeWidget(ReactComponent[btncancel.id]);
                ReactComponent[mainlayout.id].includeWidget(ReactComponent[buttonlayout.id]);
                break;
            }
            case "CreateObject": 
            {
            ...
            case "OpenPropertyEditor":
            {
                ...
                if(content.hasOwnProperty("object"))
                ...
                SeparatePropertyByGroups = function(properties)
                {
                    ...
                    for(let prototypeID = 0; prototypeID < prototypeKeys.length; prototypeID++)
                    {
                        separatedPropertiesByGroups[prototypeKeys[prototypeID]] = [];

                        const propertyKeys = Object.keys(properties[prototypeKeys[prototypeID]]);
                        for(let propertyID = 0; propertyID < propertyKeys.length; propertyID++)
                        {
                            if(prototypeKeys.length > 1)
                            {
                                if(CheckIsOwnPrototypeContainProperty(ownProperties,ownPrototypePatternID,propertyKeys[propertyID])) continue;
                            }
                            for(let groupID = 0; groupID < Object.keys(Groups).length; groupID++)
                            {
                                const groupName = Groups[groupID]["name"];
                                if(!separatedPropertiesByGroups[prototypeKeys[prototypeID]].hasOwnProperty(groupName))
                                separatedPropertiesByGroups[prototypeKeys[prototypeID]][groupName] = {};
                                console.log(propertyKeys[propertyID]);
                                if(groupName === "ALL")
                                {
                                    //debugger;
                                    separatedPropertiesByGroups[prototypeKeys[prototypeID]][groupName][propertyKeys[propertyID]] = properties[prototypeKeys[prototypeID]][propertyKeys[propertyID]];
                                ...
                                if(groupName === properties[prototypeKeys[prototypeID]][propertyKeys[propertyID]]["category"])
                                {
                                    separatedPropertiesByGroups[prototypeKeys[prototypeID]][groupName][propertyKeys[propertyID]] = properties[prototypeKeys[prototypeID]][propertyKeys[propertyID]];
                                    /*separatedPropertiesByGroups[prototypeKeys[prototypeID]][groupName][propertyKeys[propertyID]] = {
                                        "value" : properties[prototypeKeys[prototypeID]][propertyKeys[propertyID]]["value"],
                ...
                CheckIsOwnPrototypeContainProperty = function(ownProperties, ownPrototypePatternID, name, property)
                {
                    if(ownProperties.hasOwnProperty(ownPrototypePatternID) && ownProperties[ownPrototypePatternID].hasOwnProperty(Groups[0]["name"]) 
                    && ownProperties[ownPrototypePatternID][Groups[0]["name"]].hasOwnProperty(name)) return true;
                    return false;
                }

                ownProperties = SeparatePropertyByGroups(GeneratePropertiesInRightFormat(ownPrototype));
                console.log('ownPro', ownProperties);
                prototypeProperties = SeparatePropertyByGroups(GeneratePropertiesInRightFormat(prototypes));

                AddItemInPropertyList = function(propertyListID, name, property, isOwn, value = "")
                {
                    console.log('property',property);
                    const {layout,label,input} = DrawLabelWithInput(propertyListID, name, "");
                    ...
                    if(isOwn)
                    {
                        ...
                        const btn2 = AddButtonInLayout(layout, "-", () => {
                            ...
                            delete ownProperties[ownPrototypePatternID][Groups[0]["name"]][name];
                            delete ownProperties[ownPrototypePatternID][property["category"]][name];

                            if(!prototypeProperties[property["prototypeID"]].hasOwnProperty(Groups[0]["name"]))
                                prototypeProperties[property["prototypeID"]][Groups[0]["name"]] = {};

                            if(!prototypeProperties[property["prototypeID"]].hasOwnProperty(property["category"]))
                                prototypeProperties[property["prototypeID"]][property["category"]] = {};

                            prototypeProperties[property["prototypeID"]][Groups[0]["name"]][name] = property;
                            prototypeProperties[property["prototypeID"]][property["category"]][name] = property;

                            RemoveItemFromPropertyList(propertyListID,layout);
                            AddItemInPropertyList(prototypePropertiesList,name,property,false);
                        });
                    ...
                    else
                    {
                        const btn = AddButtonInLayout(layout, "+", () => {
                            ...
                            if(!ownProperties[ownPrototypePatternID].hasOwnProperty(Groups[0]["name"]))
                                ownProperties[ownPrototypePatternID][Groups[0]["name"]]= {}

                            
                            if(!ownProperties[ownPrototypePatternID].hasOwnProperty(property["category"]))
                                ownProperties[ownPrototypePatternID][property["category"]]= {}


                            ownProperties[ownPrototypePatternID][Groups[0]["name"]][name] = property;
                            ownProperties[ownPrototypePatternID][property["category"]][name] = property;

                            delete prototypeProperties[property["prototypeID"]][Groups[0]["name"]][name];
                            delete prototypeProperties[property["prototypeID"]][property["category"]][name];
                            RemoveItemFromPropertyList(propertyListID,layout);
                            AddItemInPropertyList(ownPropertiesList,name,property,true,ReactComponent[input].text);
                ...
                DrawMenuChooseGroup = function(ParentLayoutID)
                {
                    for(let i = 0; i < 8; i++)
                    {
                        const {button} = AddButtonInLayout(ParentLayoutID,i,() => {SwitchPropertiesGroup(i)});
                ...
                SwitchPropertiesGroup = function(group)
                {
                    ...
                    DrawPropertiesList(ownPropertiesList,currentGroup,ownProperties,true);
                    DrawPropertiesList(prototypePropertiesList,currentGroup,prototypeProperties,false);
                }
                DrawPropertiesList = function(parentLayoutID, group, properties, isOwn)
                {
                    const prototypeKeys = Object.keys(properties);
                    for(let i = 0; i < prototypeKeys.length; i++)
                    {
                        try{
                            let propertiesKeys = Object.keys(properties[prototypeKeys[i]][group["name"]]);
                            console.log('propertiesKeys', propertiesKeys);
                            for(let j = 0; j < propertiesKeys.length; j++)
                            {
                                AddItemInPropertyList(parentLayoutID,propertiesKeys[j],properties[prototypeKeys[i]][group["name"]][propertiesKeys[j]],isOwn);
                            }
                        }
                        catch(e)
                        {}
                ...
                DrawPropertiesByGroup = function(parrentLayoutID, group, properties, isOwn)
                {
                    let propertiesButtonLayout = new widgetsComponentsTypes["layoutHorizontal"];
                    propertiesButtonLayout.width = "500px";
                    let btn;
                    if(isOwn)
                    {
                        btn = AddButtonInLayout(propertiesButtonLayout.id,"Own Properties", function(){});
                    }
                    else 
                    {
                        btn = AddButtonInLayout(propertiesButtonLayout.id,"Prototypes Properties", function(){});
                    }
                    ReactComponent[btn["button"]].textColor = "#123456";
                    ReactComponent[parrentLayoutID].includeWidget(ReactComponent[propertiesButtonLayout.id]);

                    let propertiesListLayout = new widgetsComponentsTypes["layoutHorizontal"];
                    ...
                    ReactComponent[propertiesListLayout.id].includeWidget(ReactComponent[propertiesList.id]);
                    ReactComponent[parrentLayoutID].includeWidget(ReactComponent[propertiesListLayout.id]);
                    return {
                ...
                let objectsettingslayout = new widgetsComponentsTypes["layoutVertical"];
                objectsettingslayout.width = "500px";
                DrawLabelWithInput(objectsettingslayout.id,"Name",object["name"]);
                DrawLabelWithInput(objectsettingslayout.id,"Description",object["description"]);
                DrawLabelWithInput(objectsettingslayout.id,"Wiki",object["wiki"]);
                ReactComponent[mainlayout.id].includeWidget(ReactComponent[objectsettingslayout.id]);





                let propertiesGroupLayout = new widgetsComponentsTypes["layoutHorizontal"];
                propertiesGroupLayout.width = "500px";
                DrawMenuChooseGroup(propertiesGroupLayout.id);
                ReactComponent[mainlayout.id].includeWidget(ReactComponent[propertiesGroupLayout.id]);


                

                let ownPropertiesLayout = new widgetsComponentsTypes["layoutVertical"];
                ownPropertiesLayout.width = "500px";
                ownPropertiesList = DrawPropertiesByGroup(ownPropertiesLayout.id,currentGroup,ownProperties,true)["propertiesList"];

                let prototypePropertiesLayout = new widgetsComponentsTypes["layoutVertical"];
                prototypePropertiesLayout.width = "500px";
                prototypePropertiesList = DrawPropertiesByGroup(prototypePropertiesLayout.id,currentGroup,prototypeProperties,false)["propertiesList"];

               
                ReactComponent[mainlayout.id].includeWidget(ReactComponent[ownPropertiesLayout.id]);
                ReactComponent[mainlayout.id].includeWidget(ReactComponent[prototypePropertiesLayout.id]);
    ...
    OpenPropertyEditor(groups, openCreateHandler, saveObjectHandler)
    {
        ...
        let DrawLayout = function(parentLayoutID, type, style = {"width" : "700px", "height" : "50px"})
        {
        ...
        let DrawLabelWithInput = function(parentLayoutID, name, value = "", styleLabel = {}, styleInput = {})
        {
            ...
            for(let i = 0; i < styleLabelProperties.length; i++)
                ReactComponent[label.id][styleLabelProperties[i]] = style[styleLabelProperties[i]];
            ...
            for(let i = 0; i < styleInputProperties.length; i++)
                ReactComponent[label.id][styleInputProperties[i]] = style[styleInputProperties[i]];
        ...
        let AddButtonInLayout = function(parentLayoutID, name, style = {}, callback = null)
        {
            //debugger;
            ...
            const styleProperties = Object.keys(style);
            for(let i = 0; i < styleProperties.length; i++)
                ReactComponent[button.id][styleProperties[i]] = style[styleProperties[i]];
        ...
        let DrawChoisePropertyMenu = function(parentLayoutID)
        {
            ...
            for(let i = 0; i < groupKeys.length; i++)
            {
                const button = AddButtonInLayout(parentLayoutID,i,{"textColor" : "#123456"});
        ...
        let ReturnButtonsWidget = function(groups)
        {
            ...
            for(let i = 0; i < groupKeys.length; i++)
                ret[groups[groupKeys[i]]["name"]] = {"widget" : groups[groupKeys[i]]["widget"], "isSelected" : groups[groupKeys[i]]["isSelected"]};
            return ret;
        }
        ...
        const name = DrawLabelWithInput(DrawLayout(descobject,"layoutHorizontal"),"Name");
        const desc = DrawLabelWithInput(DrawLayout(descobject,"layoutHorizontal"),"Description");
        const wiki = DrawLabelWithInput(DrawLayout(descobject,"layoutHorizontal"),"Wiki");
        ...
        const ownPropertiesButtonLayout = DrawLayout(ownPropertiesLayout,"layoutHorizontal");
        AddButtonInLayout(ownPropertiesButtonLayout,"Own Properties",{"textColor" : "#123456"},null);
        AddButtonInLayout(ownPropertiesButtonLayout,"+",{"textColor" : "#123456"},() => {
        ...
        const ownPropertiesList = DrawLayout(DrawLayout(ownPropertiesLayout,"layoutHorizontal"),"layoutVertical");
        ...
        AddButtonInLayout(DrawLayout(prototypePropertiesLayout,"layoutHorizontal"),"PrototypeProperties",{"textColor" : "#123456"},null);
        const prototypePropertiesList = DrawLayout(DrawLayout(prototypePropertiesLayout,"layoutHorizontal"),"layoutVertical");
        ReactComponent[prototypePropertiesList].controlChildrenHorizontalAlignType(3);
    ...
    FillObjectData(data, inputs)
    {
        ...
        if(!inputs.hasOwnProperty("name") || !inputs.hasOwnProperty("desc") || !inputs.hasOwnProperty("wiki"))
        {
    ...
    InterpretationProperties(objectPrototypeProperties, prototypesProperties, Groups)
    {
        ...
        let RemoveRepeateProperties = function(ownProperties, prototypeProperties)
        {
            const ownPrototypeKeys = Object.keys(ownProperties);
            if(typeof ownPrototypeKeys === 'undefined' || ownPrototypeKeys.length === 0) return;
            if(!ownProperties[ownPrototypeKeys[0]].hasOwnProperty(Groups[0])) return;
            const ownPropertyKeys = Object.keys(ownProperties[ownPrototypeKeys[0]][Groups[0]]);

            for(let i = 0; i < ownPropertyKeys.length; i++)
            {
                const prototypeID = ownProperties[ownPrototypeKeys[0]][Groups[0]][ownPropertyKeys[i]]["prototypeID"];
                const category = ownProperties[ownPrototypeKeys[0]][Groups[0]][ownPropertyKeys[i]]["category"];
        ...
        ownProperties = SeparatePropertyByGroups(GeneratePropertiesInRightFormat(objectPrototypeProperties));
        prototypeProperties = SeparatePropertyByGroups(GeneratePropertiesInRightFormat(prototypesProperties));
    ...
    FillObjectProperties(ownList, prototypeList, objectProperties, prototypeProperties, groupName)
    {
        let DrawLayout = function(parentLayoutID, type, style = {"width" : "700px", "height" : "50px"})
        {
            if(type !== "layoutVertical" && type !== "layoutHorizontal")
        ...
        let AddItemInPropertyList = function(propertyListID, propertyName, property, isOwn)
        {
            const layout = DrawLayout(propertyListID,"layoutHorizontal",{"width" : "700px", "minHeight" : "70px"});
            if(isOwn)
            {
                ...
                AddButtonInLayout(layout,"-",{"textColor" : "#123456"},() => {
                    ...
                    delete objectProperties[Object.keys(objectProperties)[0]]["ALL"][propertyName];
                    delete objectProperties[Object.keys(objectProperties)[0]][category][propertyName];
            ...
            else 
            {
                ...
                AddButtonInLayout(layout,"+", {"textColor" : "#123456"}, () => {
                    ...
                    if(!objectProperties[Object.keys(objectProperties)[0]].hasOwnProperty("ALL"))
                        objectProperties[Object.keys(objectProperties)[0]]["ALL"] = {};

                    if(!objectProperties[Object.keys(objectProperties)[0]].hasOwnProperty(category))
                        objectProperties[Object.keys(objectProperties)[0]][category] = {};
                    
                    
                    objectProperties[Object.keys(objectProperties)[0]]["ALL"][propertyName] = property;
                    objectProperties[Object.keys(objectProperties)[0]][category][propertyName] = property;
        ...
        let DrawItems = function(propertyListID, properties, groupName, isOwn)
        {
            ...
            for(let prototypeID = 0; prototypeID < prototypeKeys.length; prototypeID++)
            {
                if(!properties[prototypeKeys[prototypeID]].hasOwnProperty(groupName)) continue;
                const propertyKeys = Object.keys(properties[prototypeKeys[prototypeID]][groupName]);
                for(let propertyID = 0; propertyID < propertyKeys.length; propertyID++)
                {
                    AddItemInPropertyList(propertyListID,propertyKeys[propertyID], properties[prototypeKeys[prototypeID]][groupName][propertyKeys[propertyID]],isOwn);
                }
            }
        }
    ...
    SetHandlerSwitchGroup(btns, handler, ownList, prototypeList, ownProperties, prototypeProperties)
    {
        const btnKeys = Object.keys(btns);
        for(let btni = 0; btni < btnKeys.length; btni++)
        {
            ReactComponent[btns[btnKeys[btni]]["widget"]].htmlElement.addEventListener("click", () => {
    ...
    OpenPropertyCreate(types,themes, groups, prototypeID, addPropertyHandler)
    {
        ...
        let DrawLabelWithCombobox = function(parentLayoutID, name, comboboxlist, labelstyle = {}, comboboxstyle = {}){
            const label = new widgetsComponentsTypes["label"];
            label.text = name;
            
            const styleLabelProperties = Object.keys(labelstyle);
            for(let i = 0; i < styleLabelProperties.length; i++)
                ReactComponent[label.id][styleLabelProperties[i]] = labelstyle[styleLabelProperties[i]];

            const combobox = new widgetsComponentsTypes["comboBox"];
       
            for(let i = 0; i < comboboxlist.length; i++)
            {
                ReactComponent[combobox.id].addItem(comboboxlist[i]["text"], comboboxlist[i]["callback"]);
            }

            const styleComboboxProperties = Object.keys(comboboxstyle);
            for(let i = 0; i < styleComboboxProperties.length; i++)
                ReactComponent[combobox.id][styleComboboxProperties[i]] = comboboxstyle[styleComboboxProperties[i]];
        ...
        const typearray = Object.keys(types).map((i) => {return types[i]["ru"]}).map((i)=>{return {"text" : i, "callback" : null}});
        const grouparray = Object.values(groups).filter((i) => {return i !== "ALL"}).map((i) => {return {"text" : i, "callback" : null}});

        const dialog = new widgetsComponentsTypes["dialog"];
        const mainlayout = DrawLayout(dialog.id,"layoutVertical",{"width" : "700px"});
        const propertydata = DrawLayout(mainlayout,"layoutVertical", {"width" : "700px"});
        const nameInput = DrawLabelWithInput(DrawLayout(propertydata,"layoutHorizontal"),"Name","")["input"];
        const wikiInput = DrawLabelWithInput(DrawLayout(propertydata,"layoutHorizontal"),"Wiki","")["input"];


        const categorybox = DrawLabelWithCombobox(DrawLayout(mainlayout,"layoutHorizontal"), "Category", grouparray)["combobox"];
        const typebox = DrawLabelWithCombobox(DrawLayout(mainlayout,"layoutHorizontal"), "Type", typearray)["combobox"];

        const valueInput = DrawLabelWithInput(DrawLayout(mainlayout,"layoutHorizontal"),"Value","")["input"];
        ...
        AddButtonInLayout(btnlayout, "Добавить", {"textColor" : "#123456"}, () => {
            ...
            let errors = [];
            if(name.length < 3)
                errors.push("Name must be more then 3 characters");
            if(wiki.length < 3)
                errors.push("Wiki must be more then 3 characters");
            ...
            const obj = {
                ...
                "wiki" : wiki,
                "category" : category,
                "type" : Object.keys(types).map((i)=>{if(types[i]["ru"] === type)return i}).filter((i) => {return i})[0],
    ...
    addOwnPropertyToPropertyList(propertyList, property){
        ...
        if(propertyList[prototypeID].hasOwnProperty("ALL") && propertyList[prototypeID]["ALL"].hasOwnProperty(property["name"])){
    ...
    saveProperty22(property, properties, event){
        ...
        const request =
            ...
            '"id": "DOLMongoRequest",' +
            '"tokens": {' +
            '	"token_long": "' + APP.dbWorker.m_long_token + '",' +
            '	"token_short": "' + APP.dbWorker.m_short_token + '"' +
            '},' +
            '"cmd": {' +
            '	"insert": "props",' +
            '	"documents": [' + JSON.stringify(objectData) + ']' +
            '	}}0f0f0f0f';
        console.log(request);
    ...
    PrepareObjectToSaveDB(objectID, objectData, objectProperties, prototypeIDs){
        ...
        object["properties"] = {};
        if(!objectProperties[prototypeID].hasOwnProperty("ALL")) return {"object": object, "prototype" : prototype};
        const propertyKeys = Object.keys(objectProperties[prototypeID]["ALL"])
        for(let i = 0; i < propertyKeys.length; i++){
            let property = {};
            property["category"] = objectProperties[prototypeID]["ALL"][propertyKeys[i]]["category"];
            property["prototypeID"] = objectProperties[prototypeID]["ALL"][propertyKeys[i]]["prototypeID"];
            property["ptype"] = objectProperties[prototypeID]["ALL"][propertyKeys[i]]["type"];
            property["prop_ref"] = objectProperties[prototypeID]["ALL"][propertyKeys[i]]["propID"];

            

            let pr = {};
            pr["prop_ref"] = objectProperties[prototypeID]["ALL"][propertyKeys[i]]["propID"];
            pr["value"] = objectProperties[prototypeID]["ALL"][propertyKeys[i]]["value"];
    ...
    findNewedProperties(propertyList){
        ...
        const newProperties = [];
        for(let i = 0; i < propertyKeys.length; i++){
            if(propertyList[prototypeID]["ALL"][propertyKeys[i]]["propID"] === "newProp"){
                propertyList[prototypeID]["ALL"][propertyKeys[i]]["name"] = propertyKeys[i];
    ...
    savePrototype(object, prototype, handler){
        ...
        if(prototype["ID"] === "newPrototype"){
            ...
            '	"update": "patterns",' +
            '   "updates": [{' +
            '   "q": "_id" : {"$oid" : "'+prototype["ID"]+'"},' +
            '	"u": ' + JSON.stringify(objectData) +
            '   }]' +
            '	}}0f0f0f0f';
...
var makeRequest = function(type, name, func, parameters) {
    ...
    req += 'db.objects.insert([{"additional":{"image":"","wiki_ref":{"en":""},"category":[""]},"meta":{"Dll_object":true,"r":["All"],"name":"';
    req += name;
    
    req += '","pattern":ObjectId("602e8135b0125500080c8192"),"owner":ObjectId("595ee3580469b6a6015312f0"),"description":"empty"},"object":{"inputs":';
    
    req += '["id",' + parameters.map(x => '"' + x + '"').toString() + '],"output":"","func":"';

    req += func + ' = function(id,' + parameters.toString() + '){const objectType = \''+type+'\'; const funcName = \'' + func +'\';';
    ...
    req += ' WidgetGeoMap.runGeoMethod(objectType, id, funcName, parameters);}","lib":ObjectId("606d7fa59367694607a3596a")}}])';
    ...
    for(let i = 0; i < content.length; i++){
        if(content[i] == '\n'){
            if(str.indexOf("gpsloc") != -1){
                ...
                _value["lon"] = parseFloat(value.substring(latIndex+7, value.length - 1));
...
var calculateSpeed = function(data){
    ...
    for(let i = 0; i < times.length - 1; i++){
        let dist = distanceInKmBetweenEarthCoordinates(data[times[i]]["gpsloc"]["lat"],data[times[i]]["gpsloc"]["lon"],data[times[i+1]]["gpsloc"]["lat"],data[times[i+1]]["gpsloc"]["lon"]);
